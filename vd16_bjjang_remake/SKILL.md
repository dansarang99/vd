---
name: vd16_bjjang_remake
description: >
  비제이짱(BJ Jang) 프레젠테이션 & 데이터 분석 마스터 리메이크 스킬 (vd16).
  원천 데이터(CSV, Excel, API) 자동 탐색적 데이터 분석(EDA), 상관분석,
  고해상도 차트 생성부터 한국어 비즈니스, 강의 교안, IR 피치덱, 제안서, 테크 아키텍처
  슬라이드를 14대 벡터 다이어그램과 Pretendard 폰트로 자동 설계하고, PowerPoint에서
  100% 개체별로 직접 편집 가능한 네이티브 DrawingML PPTX 및 발표자 스피커 노트(대본)를 생성합니다.
---

# BJ Jang Presentation & EDA Master (vd16_bjjang_remake)

> 비제이짱(BJ Jang) 실무 강의 및 재배포용으로 업그레이드된 최고급형 올인원 프레젠테이션 & 자동 EDA 데이터 분석 스킬입니다.
> 원본 hugohe3/ppt-master(MIT)와 장피엠 커스터마이즈 버전을 기반으로, **자동 EDA 엔진 & 고화질 차트 생성기**, 비제이짱 5대 시그니처 프리셋(EDA데이터분석, 강의교안, 제안서, IR, 테크), 14종 다이어그램 통합, 한국어 스피커 노트(대본) 자동 생성, 원클릭 환경 설치기를 탑재한 완전 독립형 단일 패키지입니다.

**Main Pipelines**:
1. **EDA to Presentation**: `Raw Data (CSV/Excel/API) → Automated EDA Engine → High-Res Charts → 7-Slide Deck → PPTX + Speaker Notes`
2. **Document to Presentation**: `Source Document → Create Project → [Template: bjjang] → Strategist Structured Plan → Executor → QA → PPTX`
